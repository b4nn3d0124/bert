// =============================================================
// admin_gas.js  —  BorrowSmart Google Apps Script Backend
//
// WHY ALL ACTIONS ARE IN doGet:
//   Browsers send a CORS preflight (OPTIONS) before any fetch()
//   that uses a custom header (e.g. Content-Type: application/json).
//   Google Apps Script never responds to OPTIONS, so every POST
//   from a cross-origin page fails with "No Access-Control-Allow-Origin".
//
//   The fix: the frontend uses plain GET requests with URL params
//   for everything. GET requests without custom headers are "simple
//   requests" — no preflight, no CORS block. doGet() therefore
//   handles ALL actions, including writes (add, edit, delete).
//
//   doPost() is kept for direct API / server-side callers that
//   don't have the browser CORS restriction.
// =============================================================
  ...
  try {
    ...
    if (!action) {
      return createCorsResponse({
        ...
        availableActions: [
          ...
          "editAssetSuper",  "deleteAssetSuper",
    ...
    switch (action) {
      ...
      case "editAsset":
        ...
        break;
      case "editAssetSuper":
        result = editAssetSuper(p);
        break;
      case "deleteAsset":
        result = deleteAsset(p);
        break;
      case "deleteAssetSuper":
        result = deleteAssetSuper(p);
        break;
...
function editAsset(params) {
  try {
    ...
    // Regular admin edit: name/category only.
...
function editAssetSuper(params) {
  try {
    const sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName("Assets");
    if (!sheet) return { success: false, error: "Assets sheet not found." };
    ...
    if (params.name     !== undefined) sheet.getRange(rowIdx + 1, 2).setValue(params.name);
    if (params.category !== undefined) sheet.getRange(rowIdx + 1, 3).setValue(params.category);
    if (params.status   !== undefined) sheet.getRange(rowIdx + 1, 4).setValue(params.status);
    if (params.holder   !== undefined) sheet.getRange(rowIdx + 1, 5).setValue(params.holder);
    sheet.getRange(rowIdx + 1, 7).setValue(new Date().toISOString());
    return { success: true };
  } catch (err) {
    Logger.log("[editAssetSuper] " + err);
    return { success: false, error: err.toString() };
  }
}
...
function deleteAssetSuper(params) {
  try {
    return deleteAsset(params);
  } catch (err) {
    Logger.log("[deleteAssetSuper] " + err);
    return { success: false, error: err.toString() };
  }
}
